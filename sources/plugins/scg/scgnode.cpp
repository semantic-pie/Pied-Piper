...
SCgNode::SCgNode(QGraphicsItem *parent)
    : SCgObject(parent), SCgContent(), mStructType(SCgAlphabet::StructUnknown),
      mIsContentVisible(false), mContentViewer(0), mBus(0) {
...
QRectF SCgNode::boundingRect() const {
  ...
  if (!mIsContentVisible) {
    res = QRectF(-mSize.width() / 2.f, -mSize.height() / 2.f, mSize.width(),
                 mSize.height());
  } else {
...
void SCgNode::paint(QPainter *painter, const QStyleOptionGraphicsItem *option,
                    QWidget *widget) {
  ...
  if (!mIsContentVisible) {
    painter->save();
    SCgAlphabet::getInstance().paintNode(painter, mColor, boundRect, mConstType,
                                         mPermType, mStructType);
    painter->restore();
...
bool SCgNode::isContentVisible() const { return mIsContentVisible; }
...
void SCgNode::updateContentViewer() {
  ...
  if (!contentType() == SCgContent::Empty) {
    // creating new viewer
    if (!mContentViewer)
      mContentViewer = SCgContentFactory::createViewer(contentFormat());
    Q_ASSERT(mContentViewer);
...
bool SCgNode::isContentData() const { return mContentViewer != 0; }

SCgBus *SCgNode::bus() const { return mBus; }

void SCgNode::setBus(SCgBus *bus) { mBus = bus; }
...
void SCgNode::setIdtfValue(const QString &idtf) {
  ...
  if (idtf != "") {
    if (!mTextItem) {
      ...
      mTextItem->setDefaultTextColor(
          scg_cfg_get_value_color(scg_text_element_color_normal));
    }
